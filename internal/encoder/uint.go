...
func appendUintVariable(out []byte, p uintptr, bitSize uint8) []byte {
	out = append(out, 'i', ':')
	out = _appendUint(out, p, bitSize)
	return append(out, ';')
}

// append uint to bytes bypass write barrier
func _appendUint(out []byte, p uintptr, bitSize uint8) []byte {
	var u64 uint64
	switch bitSize {
	case 8:
